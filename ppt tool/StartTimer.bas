...
Public TimeForm As Timer ' 保存窗体实例的引用
Private Const TIMER_TIME As Long = 5 * 60
Private Const ALERT_TIME As Long = 10
' 回调函数
Public Sub TimerProc(ByVal hWnd As Long, _
    ...
    If ActivePresentation.SlideShowWindow Is Nothing Then
        Slide1.Label1.Width = Slide1.W
        Slide1.Label1.Height = Slide1.H
        #If VBA7 Then
    ...
    End If
    ' 最后10秒提示
    If RemainingTime <= ALERT_TIME Then
         sndPlaySound "C:\Windows\Media\Windows Ding.wav", SND_ASYNC Or SND_FILENAME
    End If
     ' 倒计时结束处理
